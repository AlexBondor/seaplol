...
public class ItemMeasurementClassifier {
    ...
    private static boolean metricSatisfied(String measurement, Set<String> collection) {
        if (measurement.length() < 3) {
            return collection.contains(measurement);
        }
        return collection.stream().anyMatch(s -> LEVENSHTEIN_DISTANCE.apply(measurement, s) < 3);
    }
